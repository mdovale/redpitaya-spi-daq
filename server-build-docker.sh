#!/usr/bin/env bash
#
# Build Red Pitaya server (and tools) as ARM ELF using Docker.
# Use from repo root: ./server-build-docker.sh [OPTIONS]
# Output: build-docker/server (and optionally ads1278_dump in server/).
#
...
IMAGE_NAME="${IMAGE_NAME:-redpitaya-spi-daq-server-builder}"
...
Build Red Pitaya-ready server binary (ARM ELF) via Docker cross-compilation.

Options:
  --build-dir DIR    Output directory for the binary [default: build-docker]
  --image NAME       Docker image name [default: redpitaya-spi-daq-server-builder]
  --tag TAG          Docker image tag [default: latest]
  --platform PLAT    Docker platform (e.g. linux/arm/v7)
  --cflagadd FLAGS   Extra flags passed to make CFLAGADD=
...
run_shell() {
  ...
  run_cmd+=("${docker_user_args[@]}" -v "$SCRIPT_DIR:/work" -w /work/server "$IMAGE_REF" bash)
  "${run_cmd[@]}"
}
...
run_build() {
  ...
  run_cmd+=("${docker_user_args[@]}" -v "$SCRIPT_DIR:/work" -w /work/server "$IMAGE_REF")
  # Build uses sysfs GPIO backend only.
  ...
  run_cmd+=(bash -lc "make clean >/dev/null 2>&1 || true; rm -f server ads1278_dump; ${build_make_cmd} && cp server \"${container_build_dir}/server\"")
  "${run_cmd[@]}"
  if command -v file &>/dev/null; then
    ...
    if ! echo "$built_type" | grep -Eqi 'ELF.*(ARM|aarch64)'; then
      ...
      echo -e "${YELLOW}Hint: run with --rebuild or ensure Docker uses arm toolchain.${NC}" >&2
      exit 1
    fi
  fi
