#!/usr/bin/env bash
#
# Deploy the Red Pitaya server binary to a device over the network (SSH/SCP).
# Use from repo root: ./server-deploy.sh --ip <address> [OPTIONS]
#
...
Deploy the server binary to a Red Pitaya (or other ARM device) over SSH.

Options:
  --ip IP            Red Pitaya IP or hostname (or use RP_IP/REDPITAYA_IP env)
  --build-dir DIR    Build directory [default: auto-detect]
  --binary PATH      Explicit path to server binary
  --user USER        SSH user [default: root]
  --target-dir DIR   Target directory on device [default: /usr/local/bin]
  --port PORT        SSH port [default: 22]
  --force            Deploy even if binary format looks wrong
  --help             Show this help

Environment:
  REDPITAYA_IP, RP_IP, RP_HOST  Same as --ip

Examples:
  ./server-deploy.sh --ip 169.254.97.245
  ./server-deploy.sh --ip rp-foo.local --build-dir build-docker
...
if command -v file &>/dev/null; then
  ...
  if ! echo "$binary_type" | grep -Eqi 'ELF.*(ARM|aarch64)'; then
    if [[ "$FORCE" == "1" ]]; then
      ...
      echo -e "${YELLOW}Build with: ./server-build-docker.sh${NC}" >&2
...
echo -e "${GREEN}Deploying server to Red Pitaya...${NC}"
